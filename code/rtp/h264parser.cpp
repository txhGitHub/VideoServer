...
H264Parser::H264Parser(const char* filepath)
{

    this->fd = open(filepath, O_RDONLY);
    LOG_DEBUG("xinhong %s", filepath);
    assert(this->fd >= 0);
    ...
    this->ptr_mapped_file_start = this->ptr_mapped_file_cur = 
                    reinterpret_cast<uint8_t *>(mmap(nullptr, file_size, PROT_READ, MAP_PRIVATE, this->fd, 0));
...
bool H264Parser::is_start_code(const uint8_t *_buffer, int64_t buffer_len, uint8_t start_code_type)
{
